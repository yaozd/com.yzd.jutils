package com.yzd.jutils.fileExt.ReadBigFile;

/***
 *
 *
 * Created by yzd on 2018/9/6 9:51.
 */
...
//
        ...
        long pos = 0L;
        while (true) {
            Map<String, Object> res = FileUtilForBigFile.readLine(file, ENCODING, pos, NUM);
            // 如果返回结果为空结束循环
...
//                logger.info(Arrays.toString(pins.toArray()));
    ...
    /**
     * 测试BufferedRandomAccessFileReadLine读取文件--byArvin推荐很是牛B
     */
    @Test
...
//                logger.info(Arrays.toString(pins.toArray()));
    ...
    public void testCommonRead() {
        ...
        int index = 0;
        while (true) {
            List<String> pins = FileUtilForBigFile.readLine(file, ENCODING, index, NUM);
            if (CollectionUtils.isNotEmpty(pins)) {
//                logger.info(Arrays.toString(pins.toArray()));
                if (pins.size() < NUM) {
                    break;
