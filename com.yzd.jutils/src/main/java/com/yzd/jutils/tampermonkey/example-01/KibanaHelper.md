...
(function() {
  ...
  function checkStatus(worker) {
    const result = worker()
    if (!result) {
      window.$running = setTimeout(checkStatus, 500, worker)
    }
  }
  /**
  * change event
...
(function() {
  ...
  function checkStatus(worker) {
    const result = worker()
    if (!result) {
      window.$running = setTimeout(checkStatus, 1000, worker)
    }
  }
  ...
  function backendStatisticsWorker() {
    ...
    for (var i = 0; i < rowSet.length; i++) {
      ...
      $(columnSet[2]).find('span:first').replaceWith(`<span ng-non-bindable><a href="http://${domain}${path}" target="_blank">${path}</a></span>`)
    }
