...
public class MoneyUtil {
    public static final NumberFormat RMBFMT = new DecimalFormat(
            "0.00");
    public static final NumberFormat BANKFMT = new DecimalFormat(
            "###,###,###,##0.00");
    ...
    public static String amountToChinese(double n) {
        String fh = n < 0 ? "负" : "";
        String abs = RMBFMT.format(Math.abs(n));
        String val = CNValueOf(abs);
        val = fh + val.replaceAll("零分$", "整").replaceAll("零角", "零");
        if (abs.matches("^[0-9]*0\\.\\d{2}$")) {
            int jiaoPos = val.indexOf("角");
            if (jiaoPos > 0) {
                val = val.substring(0, jiaoPos - 1) + "零" + val.substring(jiaoPos - 1);
            }
        }
        return val.replaceFirst("^元零", "").replaceFirst("^元", "");
    }
     ...
     * @return
     */
    public static boolean equals(Number m1, Number m2, float... C) {
        if (m1 == null || m2 == null) {
            return false;
        }
        return Math.abs(m1.doubleValue() - m2.doubleValue()) < (C.length < 1 ? 0.01f : C[0]);
    }
